...
PATH_arcface = r'./arcface' 
arcface_wts = PATH_arcface + '/arcface-r100.wts'
arcface_trt = PATH_arcface + '/arcface-r100.engine'
# model_path = r'arcfaceresnet100-8.onnx'
# handler = insightface.model_zoo.get_model('your_recognition_model.onnx')
# handler.prepare(ctx_id=0)

app = FaceAnalysis(name='arcface_r100_v1', providers=['CUDAExecutionProvider', 'CPUExecutionProvider'])
...
#            osd_rect_params =  pyds.NvOSD_RectParams.cast(obj_meta.rect_params)
            ...
            if saved_count["stream_{}".format(frame_meta.pad_index)] % 10 == 0 and obj_meta.class_id == PGIE_CLASS_ID_FACE :
                if is_first_obj:
                    ...
                    embed = get_face_embedding(n_frame)
                    print(embed)
...
def get_face_embedding(cropped_image):
    if cropped_image is None:
        print(f"Error: Could not load image", file=sys.stderr)
        return None
    
    faces = app.get(cropped_image)
    
    if len(faces) == 0:
        print(f"No face detected", file=sys.stderr)
        return None
    
    # Return embedding of the detected face
